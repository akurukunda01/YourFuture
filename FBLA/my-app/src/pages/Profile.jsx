        ...
        <div className="flex overflow-x-auto snap-x snap-mandatory space-x-4">
            ...
            >
                <div className="flex gap-3">
                    <div>
                        {user.profile_pic ?<img
                                src={`http://localhost:8000/api${user.profile_pic}`}
                                alt="Profile"
                                className="w-15 h-15 rounded-full object-cover bg-[rgba(69,69,69)]"
                            /> : 
                            <img src ={`http://localhost:8000/api/FBLA/uploads/logo.png`} className="w-15 h-15 rounded-full object-cover bg-[rgba(69,69,69)]" alt="Image logo"/>}  
                    </div>
                    <div>
                ...
                <div className="flex justify-center mt-9">
                    {post.post_media && (<img src ={`http://localhost:8000/api/${post.post_media}`} className="max-w-100 max-h-100 " alt="Image logo"/>)}
                </div>
...
export function MyProfile(){
    ...
    useEffect(()=>{fetch(`http://localhost:8000/api/myprofile`, {
        method: "GET",
        credentials: "include",
      })
    .then((res) => {
                    ...
                    <div className="flex justify-start gap-5 md:gap-10 w-full">
                        <div className="w-24 p-3h-24 rounded-full object-cover">
                            {profile.user?.profile_pic ?<img
                                src={`http://localhost:8000/api${profile.user.profile_pic}`}
                                alt="Profile"
                                className="w-24 h-24 rounded-full object-cover"
                            /> : 
                            <img src ={`http://localhost:8000/api/FBLA/uploads/logo.png`} alt="Image logo"/>}    
                        </div>
                        <div>
                ...
                {experiencesToShow.map((exp, i) => (
                    <div key={i} className="text-left mb-4 border-b border-gray-200">
                        ...
                        <div className="flex justify-center mt-5 mb-3">
                            <img src = {`http://localhost:8000/api/${exp.exp_media}`} alt="a" className="max-w-24 max-h-24"/>
                        </div>
                    </div>
                )
                )}
