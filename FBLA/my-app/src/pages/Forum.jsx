    ...
    return(
        <div className="bg-[rgba(69,69,69)] hover:bg-[rgba(89,89,89)] max-w-xl md:max-w-3xl border-b border-gray-200 text-white py-2 px-5">
            <div className="flex justify-start gap-3 mt-3">
                <div>
                  <img src ={`http://localhost:8000/api/${post.profile_pic}`} className="h-15"/>
                </div>
            ...
            <div className="flex justify-center mt-10">
                {post.post_media &&(<img src={`http://localhost:8000/api/${post.post_media}`} className="max-w-100"></img>)}
            </div>
...
function PostModal({open, setOpen}){
    ...
    const handleSubmit = async (e) => {
        ...
        e.preventDefault();
        const res = await fetch('http://localhost:8000/api/addpost', {
        method: 'POST',
        body: data,
        credentials:"include"
        });
...
function ForumSearch({posts, setPosts}){
  ...
  useEffect(() => {
    ...
    const delayDebounce = setTimeout(() => {
      fetch(`http://localhost:8000/api/postsearch?q=${query}`,{
        credential:"include"
      })
        .then((res) => res.json())
        .then((data) => setPosts(data))
